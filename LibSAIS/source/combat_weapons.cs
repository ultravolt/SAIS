...
namespace DigitalEeel
{
	public static partial class SAIS
	{
        ...
        public static void combat_gethardpoint(t_ship ship, int hdp, ref int rx, ref int ry)
		{
			t_hull hull;
        ...
        public static int combat_findtarget(t_ship ship, int hdp)
		{
			int e;
			int s;
			int a1, a2, a3;
			...
			if (shipsystems[s].type != (short)system_types.sys_weapon)
				return -1;
        ...
        public static void combat_fire(t_ship src, int hdp, t_ship trg, int start)
		{
			t_shipweapon wep;
        ...
        public static int combat_addbeam(t_shipweapon wep, t_ship src, int hdp, t_ship trg, int start, int stg)
		{
			int c;
			int b;
			int cpu;
        ...
        public static int calc_leadangle(int tx, int ty, int vtx, int vty,
			int sx, int sy, int vsx, int vsy,
			int speed)
		{
			int a1, a2, a3;
			int r;
			...
			if (sinb > 1)
				return -1;
			a = (int)(asin(sinb) * 512 / 3.14159);
			a = (a1 - a + 1024) & 1023;

			return a;
		}

        public static int combat_addproj(t_ship src, int hdp, t_ship trg, int start)
		{
			int c=0;
			int b = 0;
			int a = 0;
			...
			combat_gethardpoint(src, hdp, ref sx, ref sy);
			combat_gethardpoint(trg, -1, ref tx, ref ty);
			...
			else if (cpu >= 2)
			{
				...
				a3 = (a2 - a1 + 1536) & 1023;
				if (a3 > 512) a3 -= 1024;
				sinb = r * Math.Sin(a3 * 3.14159 / 512) / wep.speed;
				if (sinb > 1)
					return -1;
				a = (int)(Math.Asin(sinb) * 512 / 3.14159);

				//ty = get_distance( (tx>>10)-(sx>>10), (ty>>10)-(sy>>10) );
				tx = (cos1k[(Math.Abs(a3) + 1024) & 1023] * r + cos1k[(Math.Abs(a) + 1024) & 1023] * wep.speed) / wep.speed;
				if (tx > 0)
					eta = (eta * 65536) / tx;
        ...
        public static void combat_launchstages(int p, int num, int start)
		{
			int c;
			int b;
			int a;
        ...
        public static void combat_damageship(int s, int src, int dmg, int t, t_shipweapon wep, int deb)
		{
			int sys = -1;
			int d1;
			int c;
        ...
        public static void combat_killship(int s, int t, int quiet=0)
		{
			int c;
			int sz;
			int shu = 0;
        ...
        public static int combat_addexplo(int x, int y, t_ik_spritepak spr, int spin, int size, int zoom, int start, int end, int anim=0, int cam=0)
		{
			int b, c;
