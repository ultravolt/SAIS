...
void game_over()
{
	...
	{
		textstring[(int)textstrings.STR_ENDGAME_BAR1],
		textstring[(int)textstrings.STR_ENDGAME_BAR2],
		textstring[(int)textstrings.STR_ENDGAME_BAR3],
		textstring[(int)textstrings.STR_ENDGAME_BAR4],
		textstring[(int)textstrings.STR_ENDGAME_BAR5],
		textstring[(int)textstrings.STR_ENDGAME_BAR6],
		textstring[(int)textstrings.STR_ENDGAME_BAR7],
	};
...
//	ik_print(screen, font_4x8, 8, y+=8, 0, "Other discoveries.: $%d", player.bonusdata);
	...
	sprintf(edate, textstring[(int)textstrings.STR_ENDGAME_DATEF], c, months[v].name, player.stardate/365+4580);

	sprintf(topic,  textstring[(int)textstrings.STR_ENDGAME_OVER],
					textstring[(int)textstrings.STR_ENDGAME_DATE], edate);

	sprintf(edate, textstring[(int)textstrings.STR_ENDGAME_DATEF2], months[v].name, c, player.stardate/365+4580);
	...
	if (player.system == homesystem && player.death==0) // return to hope
	{
		...
		y += 1 + interface_textbox(screen, font_4x8, bx+84, by+24+y*8, 160, 128, 0, textstring[(int)textstrings.STR_ENDGAME_MUCRON1 + (player.stardate>365*20)]);

		sprintf(texty, textstring[(int)textstrings.STR_ENDGAME_MUCRON3+2*(val<0)], abs(val));
		y += 1 + interface_textbox(screen, font_4x8, bx+84, by+24+y*8, 160, 128, 0, texty);
		y = MAX(9,y);

		sprintf(texty, textstring[(int)textstrings.STR_ENDGAME_MUCRON4+2*(val<0)], jobs[job].name);
		y += 1 + interface_textbox(screen, font_4x8, bx+16, by+24+y*8, 224, 128, 0, texty);

		if (!w)
		{
			ik_print(screen, font_4x8, bx+16, by+24+y*8, 0, textstring[(int)textstrings.STR_ENDGAME_MUCRON7]);
			Play_Sound(WAV_MUS_VICTORY, 15, 1);

			sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG1], edate);
		}
		else
		{
			...
			y += 1 + MAX(8,interface_textbox(screen, font_4x8, bx+84, by+24+y*8, 160, 128, 0, textstring[(int)textstrings.STR_ENDGAME_KAWANGI]));
			Play_Sound(WAV_KAWANGI, 15, 1);

			sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG7], edate);
		}
	}
	else		// died
	{
		...
		switch (player.death)
		{
			...
			case 1:	// combat
				sprintf(texty, 
								textstring[(int)textstrings.STR_ENDGAME_DEATH1],
								shiptypes[0].name);
				sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG2], edate);
				deathpic = spr_SMraces->spr[RC_BLOWUP];
			break;
			case 2:	// hole
				sprintf(texty,
								textstring[(int)textstrings.STR_ENDGAME_DEATH2],
								shiptypes[0].name);
			...
			case 3: // vacuum collapse
				sprintf(texty,
								textstring[(int)textstrings.STR_ENDGAME_DEATH3],
								shiptypes[0].name);
				sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG4], edate);
				deathpic = spr_SMraces->spr[RC_BLOWUP];
			break;
			case 4: // nova shockwave
				sprintf(texty,
								...
								shiptypes[0].name);
				sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG5], edate);
				deathpic = spr_SMraces->spr[RC_BLOWUP];
			break;
			case 5: // sabotage
				sprintf(texty,
								textstring[(int)textstrings.STR_ENDGAME_DEATH5],
								shiptypes[0].name);
			...
			case 6: // glory harvested by kawangi
				sprintf(texty,
								textstring[(int)textstrings.STR_ENDGAME_DEATH6],
								shiptypes[0].name);
				sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG7], edate);
			...
			case 7: // glory destroyed by vacuum collapser
				sprintf(texty,
								textstring[(int)textstrings.STR_ENDGAME_DEATH7],
								shiptypes[0].name);
				sprintf(player.deathmsg, "%s %s", textstring[(int)textstrings.STR_ENDGAME_MSG8], edate);
